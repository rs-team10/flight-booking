...
<template>
    <div id="add-rentacar">
        ...
        <div id="alerts">
            ...
            >
            RentACar service: <b>{{ rentacar.name }}</b> registered successfully.
            </v-alert>
        ...
        <div id="add-form" >
            <v-flex xs12 sm6 offset-sm3>
                <h1 class="text-xs-center indigo--text">Register new RentACar service</h1>
                <v-form>
                    <v-text-field
                    ...
                    <v-layout row justify-center>
                        ...
                        <v-card>
                            <v-card-title>
                                <span class="headline">New RentACar administrator</span>
                            </v-card-title>
                            <v-card-text>
                    ...
                    <v-layout row>
                        ...
                        <v-card>
                            ...
                            <v-toolbar-title class="text-xs-center">RentACar administrators</v-toolbar-title>
                            <v-spacer></v-spacer>
                            </v-toolbar>
                                <v-list 
                                    two-line 
                                    ...
                                    dark
                                    style="height: 400px">
                                    <template v-for="admin in this.rentacar.administrators"> 
...
export default {
    ...
    methods:{
        submit () {
            this.$v.rentacar.$touch();
            if(!this.$v.rentacar.$invalid){
                if(!this.adminEmpty()){
                    this.addRentACar();
                }else{
        ...
        save(){
            ...
            if(!this.$v.tempAdmin.$invalid){
                if(!this.adminExists(this.tempAdmin.username)){
                    this.rentacar.administrators.push(this.tempAdmin);
                    this.close();
                }
                else{
        ...
        adminExists(usr){
            for(var i=0; i < this.rentacar.administrators.length; i++){
                if( this.rentacar.administrators[i].username == usr)
                    return true
            }
            return false
        },
        adminEmpty(){
            if(this.rentacar.administrators.length == 0)
                return true;
            else
                return false;
        },
        deleteAdmin(admin){
            this.rentacar.administrators.pop(admin);
        }
    }
}
</script>
